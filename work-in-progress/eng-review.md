...
3. **Complexity check:** If the plan touches more than 8 files, treat that as a signal worth questioning — but in a domain-first layout, multiple focused classes per bounded context is expected and healthy (e.g. `PaymentAuthorisationService`, `PaymentAuthorisationController`, `AuthorisationResult` are three classes for one feature, not three features). The real smell is new classes that cross domain boundaries or introduce layers with no caller-visible value. Challenge *cross-cutting* additions, not the count of domain-aligned classes.
...
* **`@SpringBootTest`** — for full integration tests only; treat using this for every test as a smell (slow, tests too much at once)
* **`@ApplicationModuleTest`** (Spring Modulith) — for verifying module boundaries and cross-module event contracts
* **ArchUnit** — for enforcing layer rules and domain boundary constraints across the whole codebase
