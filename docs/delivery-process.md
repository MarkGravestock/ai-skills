...
artifact and ceremony overhead — the same instinct behind this repo's
...
- **Walking skeleton first.** Before depth in any one component, get the
  ...
  This is [whole-product thinking](https://less.works/less/principles/whole-product-focus)
  taken to its extreme: customers don't buy a part of the product, so a
  skeleton that is thin everywhere but whole beats a component that's deep
  but disconnected.
- **MVP slicing.** Cut vertically, by the smallest thing that produces real
  ...
  other end perceives, not by what's convenient to build —
...
time on their own, and if the extra headroom gets spent opening more
branches or running more agent tasks in parallel, work-in-progress rises and
cycle time gets *worse*, not better, even though everyone is typing less.

The corollary: cap how much is in flight at once, and put the time agents
...
unvalidated work in progress, and the [Lean
Thinking](https://less.works/less/principles/lean-thinking) framing is the
same: optimize the whole stream and remove the bottleneck, don't maximize
...
but speed in Act alone doesn't shorten the loop unless Observe and Orient
keep pace.
...
a single cross-team backlog, feature-team structure, one product owner. None
of that applies here; there's no multi-team coordination problem to solve.
What's adopted is the perspective (flow, systems thinking, empirically
verified small increments, customer-defined value), not the Scrum-at-scale
mechanics that principle exists to support.
